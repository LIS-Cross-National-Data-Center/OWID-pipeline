...
print("==============================")
print(paste0("Initiated 2.3 at ", format(Sys.time(), "%d-%B-%Y %H:%M:%S")))
print("==============================")
...
gini <- prep_data_mi %>% #list
  lissyrtools::run_weighted_gini("mi", "new_wgt") %>% # dedicated function, that only asks for a variable and a weight
...
share_richest_10 <- prep_data_mi %>%
  lissyrtools::run_weighted_percentiles("mi", "new_wgt", probs = c(0.9), share = TRUE) %>%
...
share_poorest_50 <- prep_data_mi %>%
  lissyrtools::run_weighted_percentiles("mi", "new_wgt", probs = c(0.5), share = TRUE) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
share_below_50_median <- prep_data_mi %>%
  lissyrtools::run_weighted_relative_poverty("mi", "new_wgt", times_median = 0.5) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
  mutate(indicator = "Share below half median")
...
average <- prep_data_ppp_adj_mi %>%
  lissyrtools::run_weighted_mean("mi", "new_wgt") %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
median <- prep_data_ppp_adj_mi %>%
  lissyrtools::run_weighted_percentiles("mi", "new_wgt", probs = c(0.5)) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
deciles <- prep_data_ppp_adj_mi %>%
  lissyrtools::run_weighted_percentiles("mi", "new_wgt", probs = seq(0.1, 0.9, 0.1)) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
print("==============================")
print(paste0("Finished 2.3 at ", format(Sys.time(), "%d-%B-%Y %H:%M:%S")))
print("==============================")