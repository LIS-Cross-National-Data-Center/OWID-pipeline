...
# 1) Inequalities  --------------------------------------
gini <- prep_data %>% #list
  lissyrtools::run_weighted_gini("dhi_pc", "new_wgt") %>% # dedicated function, that only asks for a variable and a weight
...
share_richest_10 <- prep_data %>%
  lissyrtools::run_weighted_percentiles("dhi_pc", "new_wgt", probs = c(0.9), share = TRUE) %>%
...
share_poorest_50 <- prep_data %>%
  lissyrtools::run_weighted_percentiles("dhi_pc", "new_wgt", probs = c(0.5), share = TRUE) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
share_below_50_median <- prep_data %>%
  lissyrtools::run_weighted_relative_poverty("dhi_pc", "new_wgt", times_median = 0.5) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
  mutate(indicator = "Share below half median")
...
average <- prep_data_ppp_adj %>%
  lissyrtools::run_weighted_mean("dhi_pc", "new_wgt") %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
median <- prep_data_ppp_adj %>%
  lissyrtools::run_weighted_percentiles("dhi_pc", "new_wgt", probs = c(0.5)) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
deciles <- prep_data_ppp_adj %>%
  lissyrtools::run_weighted_percentiles("dhi_pc", "new_wgt", probs = seq(0.1, 0.9, 0.1)) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>%
...
percentiles <- prep_data_ppp_adj %>%
  lissyrtools::run_weighted_percentiles("dhi_pc", "new_wgt", probs = seq(0.01, 0.99, 0.01)) %>%
  lissyrtools::structure_to_plot(print_columns = FALSE) %>% 
